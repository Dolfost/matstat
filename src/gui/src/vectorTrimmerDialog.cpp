...
void VectorTrimmerDialog::trim() {
	Vector* newEntry = new Vector(new ss::Vector(ve->vector()->list()));
	size_t delta = newEntry->vector()->trim(glow, ghigh);
	newEntry->setName("TRIM" + QString::number(++trimIdx) + "(" + ve->name() + ")");
...
void VectorTrimmerDialog::vectorDeletedHandler(Vector* vectorEntry) {
	if (ve == vectorEntry)
		this->close();
}
